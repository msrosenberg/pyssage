...
def threetlqv(transect: numpy.ndarray, min_block_size: int = 1, max_block_size: int = 0, block_step: int = 1,
              wrap: bool = False, unit_scale: Number = 1) -> numpy.ndarray:
    """
    ...
    # if max_block_size == 0:
    #     max_block_size = n // 3
    #     # print("Maximum block size cannot exceed 33% of transect length. Reduced to", max_block_size)
    # if max_block_size < 2:
    #     max_block_size = 2
    for b in range(min_block_size, max_block_size + 1, block_step):
        cnt = 0
...
begin
     ...
     for b := 1 to maxb do if ContinueProgress then begin
         ...
         else endp := DatMat.nrows - 2 * b;
         for i := 1 to endp do if ContinueProgress then begin
             ib := WrapTransect(i+b,DatMat.nrows);
             i2b := WrapTransect(i+2*b,DatMat.nrows);
             if DatMat.IsNum[i,1] and DatMat.IsNum[ib,1] and DatMat.IsNum[i2b,1] then begin
                inc(cnt);
                OutMat[b,outcol] := OutMat[b,outcol] + sqr(DatMat[i,1] - 2.0 * DatMat[ib,1] + DatMat[i2b,1]);
             end;
         end;
         if (cnt > 0) then
            OutMat[b,outcol] := OutMat[b,outcol] / (4.0 * cnt)
...
procedure Calc_2NLV(DatMat,OutMat : TpasMatrix; outcol,maxxb : integer;
          range : double; DoWrap : boolean);
var
   endp,jj,
   cnt,i,j,b,maxb : integer;
   term1,term2,sum1,sum2 : double;
   good : boolean;
begin
     maxb := trunc(DatMat.nrows * range);
     maxb := Min(maxb,maxxb);
